...
/*
        ...
        Utils.SetTarget(ref role.ClosestPlayer, role.InquireButton);
                    ...
                    var flag = PlayerControl.AllPlayerControls.ToArray().Count(p => role.Heretics.Contains(p.PlayerId)
                        && (p.Data.IsDead || p.Data.Disconnected)) == role.HereticCount;

            if (flag && !role.Invincible)
            {
                var transform = false;
                ...
                {
                    role.HereticsDead();
                    Utils.Rpc(CustomRPC.InquisitorGoal, PlayerControl.LocalPlayer.PlayerId);
                }
            }
    }
}
