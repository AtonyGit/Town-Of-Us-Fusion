...
public class HudGuardUpdate
{
    ...
    public static void UpdateGuardButton(HudManager __instance)
    {
        ...
        {
            role.UsesText = Object.Instantiate(guardButton.cooldownTimerText, guardButton.transform);
        ...
        role.UsesText.gameObject.SetActive((__instance.UseButton.isActiveAndEnabled || __instance.PetButton.isActiveAndEnabled)
                && !MeetingHud.Instance && !PlayerControl.LocalPlayer.Data.IsDead
                && AmongUsClient.Instance.GameState == InnerNet.InnerNetClient.GameStates.Started);
        if (role.Guarding) guardButton.SetCoolDown(role.TimeRemaining, CustomGameOptions.GuardDuration);
        else if (role.ButtonUsable) guardButton.SetCoolDown(role.GuardTimer(), CustomGameOptions.GuardCd);
        else guardButton.SetCoolDown(0f, CustomGameOptions.GuardCd);


            Utils.SetTarget(ref role.ClosestPlayer, __instance.KillButton, float.NaN);
        var renderer = guardButton.graphic;
        ...
        {
            if (role.Guarding || (!guardButton.isCoolingDown && role.ButtonUsable))
            {
                renderer.color = Palette.EnabledColor;
                renderer.material.SetFloat("_Desat", 0f);
                role.UsesText.color = Palette.EnabledColor;
