...
namespace TownOfUsFusion.CrewmateRoles.HaunterMod
{
    ...
    public class RepickHaunter
    {
        private static void Postfix(HudManager __instance)
        {
            ...
            {
                var toChooseFromAlive = PlayerControl.AllPlayerControls.ToArray().Where(x => x.Is(Faction.Crewmates) && !x.Is(AllianceEnum.Lover) && !x.Data.Disconnected).ToList();
            ...
            var toChooseFrom = PlayerControl.AllPlayerControls.ToArray().Where(x => x.Is(Faction.Crewmates) && !x.Is(AllianceEnum.Lover) && x.Data.IsDead && !x.Data.Disconnected).ToList();
