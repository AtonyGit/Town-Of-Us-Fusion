...
/*
    public static List<string> GenerateDownloadList(List<CustomHat> hats)
    {
        ...
        {
            if (hat.StreamOnly && !TownOfUsFusion.IsStream)
                continue;

            if (!File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.ID}.png")))
                markedfordownload.Add(hat.ID);

            if (hat.BackID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.BackID}.png")))
                markedfordownload.Add(hat.BackID);

            if (hat.ClimbID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.ClimbID}.png")))
                markedfordownload.Add(hat.ClimbID);

            if (hat.FlipID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.FlipID}.png")))
                markedfordownload.Add(hat.FlipID);

            if (hat.BackFlipID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.BackFlipID}.png")))
                markedfordownload.Add(hat.BackFlipID);

            if (hat.FloorID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.FloorID}.png")))
                markedfordownload.Add(hat.FloorID);

            if (hat.FloorFlipID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.FloorFlipID}.png")))
                markedfordownload.Add(hat.FloorFlipID);

            if (hat.ClimbFlipID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Hats", $"{hat.ClimbFlipID}.png")))
