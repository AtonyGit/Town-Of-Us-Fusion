...
/*
    public static List<string> GenerateDownloadList(List<CustomVisor> visors)
    {
        ...
        {
            if (visor.StreamOnly && !TownOfUsFusion.IsStream)
                continue;

            if (!File.Exists(Path.Combine("TownOfUsFusion.Resources.Visors", $"{visor.ID}.png")))
                markedfordownload.Add(visor.ID);

            if (visor.FlipID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Visors", $"{visor.FlipID}.png")))
                markedfordownload.Add(visor.FlipID);

            if (visor.ClimbID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Visors", $"{visor.ClimbID}.png")))
                markedfordownload.Add(visor.ClimbID);

            if (visor.FloorID != null && !File.Exists(Path.Combine("TownOfUsFusion.Resources.Visors", $"{visor.FloorID}.png")))
